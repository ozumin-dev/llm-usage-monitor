...
| 取得間隔 | 使用量を取りに行く間隔｡Codex 60-3600秒(既定60)､Claude 30-3600秒(既定300)､Antigravity 60-3600秒(既定300) |

Claudeの下限が30秒なのは､使用量のエンドポイントがそれより短い間隔だと `HTTP 429` を返すためです｡Codexは取得のたびに `codex app-server` を起動するので60秒を下限にしています｡
...
以前の設定ファイルにあった `local_refresh_seconds`(旧｢ローカル表示･Codex｣)は､Codexの取得間隔の初期値として引き継ぎます｡同じ値は取得済みファイルの定期的な読み直し間隔としても残っていますが､設定画面には出しません｡
